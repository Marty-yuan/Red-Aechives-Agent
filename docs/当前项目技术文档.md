...
- 当前数据规模：18 个云南红军长征档案 TXT 文件（约 9.8 MB）。
- 文本预处理：繁体转简体（OpenCC）、空行合并、OCR 页码噪声清理、**OCR 高频错字纠错表**（`src/knowledge/ocr_fixes.py`，53 条，接入 `rebuild_index.py`）。
- 错字治理：`scripts/find_ocr_variants.py` 以专名表扫描全库错字候选；`scripts/ocr_quality_check.py` 输出逐文件噪声统计。详见 `docs/数据质量评估报告.md`。
- 检索索引：基于 scikit-learn TF-IDF 构建字符级向量索引（2-4 gram，3000 维）。
- 索引文件：`data/index/chunks.json`（含 `source`/`offset`/`page` 溯源字段）、`vectorizer.pkl`、`embeddings.npz`、`village_index.json`。
- 溯源字段：`source`（档案名）+ `offset`（文本定位）+ `section`（篇目名）+ `page`（页码）+ `confidence`（exact/approx），由 `src/knowledge/toc_index.py` 依据档案目录 `data/knowledge_graph/book_toc.json`（1225 条篇目-页码）标注，9,132/9,132 chunk 全覆盖。
...
1. **手工结构化基础图谱**：
   ...
   - 当前规模：473 个实体、282 条关系（person / location / event / army / organization / document 等类型，commander、occurred_at、next_on_route、participant、alias 等 10+ 类），由手工基础图谱 + LLM 全量抽取（18 部档案）+ LLM 实体校验构建，经引用完整性校验。
...
```text
项目根目录
├── build_pipeline.py                 # PDF 提取（保留页码标记）+ 清洗 + 索引（备用管道）
...
│   ├── knowledge/
│   │   ├── rebuild_index.py          # 重建索引（纠错 + 篇目标注 + TF-IDF）
│   │   ├── ocr_fixes.py              # OCR 高频错字纠错表（67 条）
│   │   ├── toc_index.py              # 档案篇目目录解析与溯源标注
...
- 优先读取环境变量 `RED_ARCHIVE_PROJECT_DIR`（见 `.env.example`）；
- 未设置时按代码位置自动推导（`src/agent/config.py` 的上级上级目录即项目根）。
...
6. **OCR 错字覆盖率**：纠错表（67 条）覆盖高频错字，正文分散错字约 70-80% 未覆盖（详见 `docs/数据质量评估报告.md`），后续建议 LLM 批量纠错。
7. **API Key 与档案数据不应提交**：`.gitignore` 已配置；原始 PDF（405MB）与 OCR 文本仅保留在团队本地。
...
- 使用 `.env.example` 说明环境变量（含 `RED_ARCHIVE_TOC_XLSX` 等可选变量）。
- 将本技术文档、数据质量评估报告、检索评测报告与后续规划文档提交到 `docs/`。
- 早期脚本（`run_mineru.py`、`src/ocr/pdf_processor*` 等）已清理完毕，`scripts/cleanup_legacy.ps1` 可复用于其他机器。
